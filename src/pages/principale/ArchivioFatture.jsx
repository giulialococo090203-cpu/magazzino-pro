...
export default function ArchivioFatture() {
  ...
  const selectedVisibleIds = useMemo(() => {
    return selectedIds.filter((id) => filteredInvoiceIds.includes(id));
  }, [selectedIds, filteredInvoiceIds]);
  ...
  const selectAllVisible = () => {
    setSelectedIds((prev) => [...new Set([...prev, ...filteredInvoiceIds])]);
  };

  const deselectAllVisible = () => {
    setSelectedIds((prev) => prev.filter((id) => !filteredInvoiceIds.includes(id)));
  };
  ...
  const handleDeleteSelected = async () => {
    if (!canDelete || selectedVisibleIds.length === 0) return;

    try {
      setActionLoading('delete-selected');

      const result = await invoiceImportStore.deleteManyWithFiles(selectedVisibleIds);

      await adminLogStore.create({
        userId: user?.id,
        entity: 'fattura',
        action: 'Eliminazione fatture archiviate',
        details: `Eliminati ${result.invoicesDeleted} documenti dall’archivio fatture. File Storage eliminati: ${result.filesDeleted}/${result.filesRequested}.`,
      });
      ...
      console.error('Errore eliminazione fatture selezionate:', err);
      alert(err.message || 'Errore durante l’eliminazione delle fatture selezionate.');
    } finally {
      setActionLoading('');
    }
  };
  ...
  return (
    <div className="animate-slideUp">
      ...
      {canDelete && !loading && filteredInvoices.length > 0 && (
        <div
          className="card"
          style={{
            marginBottom: 16,
        ...
        >
          ...
          >
            <div className="text-sm" style={{ fontWeight: 800 }}>
              {selectedVisibleIds.length} documenti selezionati su {filteredInvoices.length} visibili
            </div>
          ...
          <table className="data-table">
            ...
            <tbody>
              {filteredInvoices.length === 0 ? (
                <tr>
                  <td colSpan={canDelete ? 12 : 11} style={{ padding: 40 }}>
                    <div className="empty-state">
          ...
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="modal-body" style={{ textAlign: 'center' }}>
              ...
              <p className="confirm-message">
                ...
                <br />
                Verranno eliminati davvero il file da Supabase Storage e il relativo record.
                <br />
            ...
            <div className="modal-footer">
              ...
              >
                {actionLoading === `delete-${confirmDelete.id}` ? 'Eliminazione...' : 'Elimina documento'}
              </button>
            </div>
          </div>
        </div>
      ...
      {confirmDeleteSelected && (
        <div className="modal-overlay confirm-dialog" onClick={() => setConfirmDeleteSelected(false)}>
          <div className="modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h3 className="modal-title">Eliminare documenti selezionati?</h3>

              <button className="modal-close" onClick={() => setConfirmDeleteSelected(false)}>
                ✕
              </button>
            </div>
            ...
            <div className="modal-body" style={{ textAlign: 'center' }}>
              ...
              <p className="confirm-message">
                ...
                <br />
                Verranno eliminati davvero i file da Supabase Storage e i relativi record archivio.
                <br />
                <strong>Materiali e movimenti non verranno modificati.</strong>
              </p>
            </div>
            ...
            <div className="modal-footer">
              ...
              >
                {actionLoading === 'delete-selected' ? 'Eliminazione...' : 'Elimina selezionate'}
              </button>
            </div>
          </div>
        </div>
